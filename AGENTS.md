...
### 3.3.1. Visibility policy

Для всех методов, функций, структур, enum и полей нужно использовать минимально возможную публичность.

Базовый порядок выбора:

- private (без `pub`), если элемент используется только внутри текущего модуля;
- `pub(crate)`, если элемент нужен в пределах текущего crate;
- `pub`, только если элемент действительно является частью внешнего публичного API.

Расширять область видимости “на будущее” запрещено. Публичность повышается только под подтвержденный текущий use case.
...
Файлы должны оставаться небольшими и иметь одну ясную зону ответственности.

Ориентиры:

- обычный Rust-файл: до 400 строк;
- Rust-файл с unit-тестами рядом с кодом: до 600 строк;
- большие integration/e2e-тесты должны выноситься в отдельные файлы в `tests/` или в scenario files;
- тестовый файл больше 600 строк требует разбиения или явного обоснования;
- файл больше 700 строк требует явного обоснования в отчете этапа;
- файл больше 1000 строк запрещен без отдельного архитектурного решения.
...
- временные spike-файлы, если это прямо указано в ТЗ этапа.

Unit-тесты рядом с кодом должны проверять локальные инварианты модуля.

Большие workflow/e2e-проверки не должны раздувать production-файл. Их нужно выносить в:

- `tests/*.rs` для Rust integration tests;
- `mods/test_scenarios/**` для игровых scenario tests;
- `fixtures/**` для входных данных;
- snapshot-файлы, если проверяется большой ожидаемый вывод.

Если файл начинает превышать рекомендуемый размер, нужно рассмотреть разбиение на модули, helper-файлы, fixtures или scenario files.

Запрещено увеличивать файл только потому, что “так быстрее”.
